...
filtered_cars = [(brand_name, (color, year, engine_volume, car_type, price))
                 for brand_name, (color, year, engine_volume, car_type, price) in car_data.items()
                 if year >= search_criteria[0]
                 and engine_volume >= search_criteria[1]
                 and price <= search_criteria[2]]

filtered_cars.sort(key=lambda x: x[1][4])    # відсортували за ціною: [1] - це об'єкт ('blue', 2021, 1.6, 'hatchback', 25000), а індекс [4] - це останній - 'price'
first_five_car_brands = filtered_cars[:5]
final_searching_result = [brand_name[0] for brand_name in first_five_car_brands]
print(f'Автомобілі, які відповідають {search_criteria[0]} року випуску(та пізніше),\n'
      f'з обʼємом двигуна {search_criteria[1]}(та більше),\n'
      f'та ціною, яка дорівює(та менше) {search_criteria[2]},\n'
      f'будуть наступні бренди:\n'
      f'{final_searching_result}')
